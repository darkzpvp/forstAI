  ...
  return (
    <div className="grid lg:grid-cols-3 md:grid-cols-2 h-screen ">
      ...
      <form noValidate onSubmit={handleSubmit} className="w-full max-w-72 lg:max-w-80 ">
        ...
        />
  {errores
            ? errores.map((error, i) => <div className="mb-2 text-sm text-red-600 ">{error}</div>)
            : null}
...
{loading ? (
            <button
              type="button"
              className={` cursor-not-allowed bg-[#727ee4]  max-w-72 text-white  rounded-lg text-sm px-4 py-2 w-full mt-6`}
              disabled
            >
              <svg
                role="status"
