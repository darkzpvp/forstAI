...
const [pageNumber, setPageNumber] = useState("1")
  ...
  const per_page = searchParams.get('per_page') ?? '13'
  ...
  const handleChange = () => {
    ...
    if (newPage !== page) {
        router.push(`/tablapeliculas/?page=${newPage}&per_page=${per_page}`);
    }
    setPageNumber("");
};
useEffect(() => {
  ...
  return (
    <div className='mt-2 flex gap-2 text-sm '>
      <button
        ...
        onClick={() => {
          router.push(`/tablapeliculas/?page=${Number(page) - 1}&per_page=${per_page}`)
        }}>
        Página anterior
      </button>
