...
const PaginationControls: FC<PaginationControlsProps> = ({
  ...
  const handleChange = () => {
    ...
    if (newPage !== page) {
      router.push(`/admin/?page=${newPage}&per_page=${per_page}`, { scroll: false });
    }
    setPageNumber("");
  };
  useEffect(() => {
  ...
  return (
    <div className=" p-3 mt-2 flex gap-2 text-sm ">
      ...
      <button
        ...
        onClick={(e) => {
          router.push(
            `/admin/?page=${Number(page) - 1}&per_page=${per_page}`, { scroll: false }
      ...
      <div className="flex items-center justify-center px-4 h-10 leading-tight       bg-gray-900  text-gray-400 hover:bg-gray-900 hover:text-gray-200">
        {page} / {totalPages}
      </div>
      <button
      ...
      >
        <input
          ...
          className="px-2 h-10   rounded-lg bg-gray-900 text-gray-400"
        />
